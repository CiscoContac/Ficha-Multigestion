...
%>          
<iframe id="resultadocabecera"  frameborder="NO"   scrolling ="no"  name="resultadocabecera" src="cabeceraResultado.jsp?campana=<%=campana%>&campa=<%=campa%>&codcli=<%=codcli%>"  width="1600px"  height="300px" >
 </iframe>  
 <iframe id="resulContenido"  frameborder="NO"   scrolling ="no"   name="resulContenido" src="CuerpoResultado.jsp?campana=<%=campana%>&campa=<%=campa%>&codcli=<%=codcli%>" width="1600px"  height="950px">
</iframe>
 <input type="hidden" id="txtcodtitu" name="txtcodtitu" value="<%=codcli%>">
 </form>
<script language="javascript">   
 
   //   alert("ca,bia titulo");
   var codigo=document.getElementById('txtcodtitu').value;
  // alert(codigo);
   if(codigo=='null'){
        document.title='Resultados de Contacto'
   }else{
   document.title='Resultados de Contacto - '+ codigo;  
   }
